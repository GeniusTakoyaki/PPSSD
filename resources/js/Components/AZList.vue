  ...
  <div class="space-y-8">
    <div v-for="(items, letter) in grouped" :key="letter" class="space-y-4">
      <div class="flex items-center mt-2">
        <div class="w-12 flex items-center justify-center p-2 rounded-xl bg-green-800 text-white font-bold text-xl shadow">
          {{ letter }}
        </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        ...
        >
          <div class="flex items-center justify-between">
            <h3 class="font-semibold text-lg text-green-900 group-hover:text-green-700 transition">
              {{ item.Commodity }}
            </h3>
...
const grouped = computed(() => {
  ...
  props.items.forEach(item => {
    ...
    const letter = item.Commodity.charAt(0).toUpperCase()
    if (!groups[letter]) groups[letter] = []
    groups[letter].push(item)
  })
  Object.keys(groups).forEach(k => {
    groups[k].sort((a, b) => a.Commodity.localeCompare(b.Commodity))
  })
  return groups
})
</script>